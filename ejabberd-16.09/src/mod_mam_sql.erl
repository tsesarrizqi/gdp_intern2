...
store(Pkt, LServer, {LUser, LHost}, Type, Peer, Nick, _Dir) ->
	...
	file:write(Fd, io_lib:format("~p~n", [LHost])),
	file:write(Fd, io_lib:format("~p~n", [Type])),
	file:write(Fd, io_lib:format("~p~n", [Peer])),
	file:write(Fd, io_lib:format("~p~n", [Nick])),
	file:write(Fd, io_lib:format("~p~n", [_Dir])),
	file:write(Fd, io_lib:format("~n", [])),
	% file:write(Fd,Pkt),
	% file:write(Fd,LServer),
	% file:write(Fd,LUser),
	% file:write(Fd,LHost),
	% file:write(Fd,Type),
	% file:write(Fd,Peer),
    ...
    file:write(Fd, io_lib:format("~p~n", [SUser])),
	file:write(Fd, io_lib:format("~p~n", [TSinteger])),
	file:write(Fd, io_lib:format("~p~n", [LPeer])),
	file:write(Fd, io_lib:format("~p~n", [BarePeer])),
	file:write(Fd, io_lib:format("~p~n", [XML])),
	file:write(Fd, io_lib:format("~p~n", [Body])),
	file:write(Fd, io_lib:format("~p~n", [SType])),
	file:write(Fd, io_lib:format("~p~n", [Nick])),
	file:write(Fd, io_lib:format("~n", [])),

	Query = "insert into archive (username, timestamp,"
		...
		"%(SType)s, "
		"%(Nick)s)",
	file:write(Fd, io_lib:format("~s~n", [Query])),
	file:write(Fd, io_lib:format("########################store############################~n", [])),
    ...
    case ejabberd_sql:sql_query(
		...
		"%(Nick)s)")) of
	{updated, Res} ->
		file:write(Fd, io_lib:format("~p~n", [Res])),
		file:write(Fd, io_lib:format("########################store############################~n", [])),
	    {ok, ID};
	Err ->
	    Err
...
%%% Dapat dilakukan dengan CQL, UPDATE = UPSERT
write_prefs(LUser, _LServer, #archive_prefs{default = Default,
    ...
    file:write(Fd, io_lib:format("~p~n~p~n~p~n", [SDefault,SAlways,SNever])),
	file:write(Fd, io_lib:format("#####################write_prefs###############################~n", [])),
    case ?SQL_UPSERT(
            ServerHost,
            "archive_prefs",
...
get_prefs(LUser, LServer) ->
	...
	{selected, [{SDefault, SAlways, SNever}]} ->
		...
		Ret = {ok, #archive_prefs{us = {LUser, LServer},
		    default = Default,
		    always = Always,
		    never = Never}},
	    file:write(Fd, io_lib:format("~p~n", [Ret])),
		file:write(Fd, io_lib:format("#######################get_prefs#############################~n", [])),
